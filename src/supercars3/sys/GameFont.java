...
public class GameFont
{
	...
	private static final int NB_LETTERS = 1<<16;
	...
	public GameFont(String prefix,Color original_white, Color white_replacement) throws IOException
	{
		BufferedImage image = ImageIO.read(new File(DirectoryBase.get_font_path() + prefix+".png"));
	...
	public GameFont(String prefix,Color monochrome_replacement) throws IOException
	{
		BufferedImage image = ImageIO.read(new File(DirectoryBase.get_font_path() + prefix+".png"));
		
		int orig_rgb = 0xFF; // blue, alpha channel
	...
	{
		ParameterParser fr = ParameterParser.open("fonts" + File.separator + prefix+".sc3");
	...
	public void write(Graphics2D g, String text, int x, int y, int letter_horizontal_offset, boolean horiz_centered, boolean vertical_centered, int y_offset)
	{
